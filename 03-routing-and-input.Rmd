...
El primer trabajo de plumber es ejecutar codigo R en respuesta a pedidos
HTTP entrantes, por lo que es importante entender cómo pedidos HTTP entrantes
son traducidos en la ejecución de funciones de R.

Un pedido HTTP entrante debe de ser *("ruteado")* a una o más funciones de R.
Plumber tiene dos familias distintas de funciones que lo gestionan: filtros y
*(endpoints)*

Normalmente, cuando un pedido llega a un *(router)* de Plumber, este empieza
pasando ese pedido a través de sus filtros. Una vez que el pedido ha sido
procesado por todos los filtros, el *(router)* buscará un *(endopoint)* que
pueda satisfacer el pedido entrante. Si encuentra uno, invocará el *(endpoint)*
y responderá a los pedidos entrantes usando el valor del *(endpoint)* devuelto.
Si ningun *(endpoint)* concuerda con el pedido, entonces un error de tipo
`404 Not Found` será mostrado. (El comportamiento de este puede ser controlado
por el método [`set404Handler`](#customize-router))

## Endpoints {#endpoints}
*(pedido o petición)*
Los *(Endpoints)* son un paso terminal en el proceso de servir un pedido. Un
*(endpoint)* puede ser visto como simplemente la lógica que es la responsable 
final de generar una respuesta a una petición en particular. Una petición puede 
ser revisada contra cada endpoint disponible hasta que encuentre uno dispuesto
a servirle en cuyo momento dejará de buscar. *(i.e.)* una petición no será
procesada por más de un endpoint. Puedes crear un endpoint escribiendo una 
función tal como:
...
Esta *(annotatio)* especifíca que ésta función es responsable por generar la
respuesta a cualquier pedido `GET` hacia `/hello`. El valor retornado por la 
función será  usado como la respuesta al pedido (después de haber sido
corrido a través de un serializador para *(e.g.)* convertir la respuesta en un
JSON). En este caso, una respuesta `GET` a `/hello` retornará el contenido
`["hello world"]` con un contenido tipo `JSON`.

Las *(annotations)* que generan un endpoint incluyen:
...
*¿?* Este mapa de los métodos de HTTP que un cliente API puede enviar en conjunto
con una petición. Por defecto cuando se abre una página en un navegador web, 
se envía una petición `GET` al API. Sin embargo puedes usar otros clientes API
(o inclusive JavaScript dentro de un navegador web) para formular peticiones
HTTP usando los otros métodos listados aquí. Existen convenciones sobre cuándo
cada uno de estos métodos debe de ser usados, para lo cual puedes leer más 
[aquí](http://www.restapitutorial.com/lessons/httpmethods.html). Tome en cuenta
que cada una de estas convenciones acarrea consigo implicaciones de seguridad, 
por lo que es una buena idea seguir los usos recomendados para cada método hasta
que comprenda completamente porqué podría  *(deviate)* de estos.

Tome en cuenta que un endpoint en particular puede *(support)* múltiples verbos.
La siguiente función será usada para servir a cualquier petición `GET`, `POST`
o `PUT` petición para  `/cars`.
...
Los filtros  de Plumber pueden ser usados para definir un *(pipeline)* para
manejar peticiones entrantes. Esto permite a los autores del API segmentar
lógica compleja en una secuencia de pasos independientes  y comprensibles.
A diferencia de los endpoints, una petición puede pasar a través de múltiples
filtros  de Plumber previo a que una respuesta sea generada.

Genermente, un router de Plumber pasará una petición a través de todos los
filtros definidos previo a buscar un endpoint que satisfaga la petición. Sin 
embargo, los endpoints pueden  *(preempt)* filtros en particular si ellos
quieren ser considerados para la ejecución previo a que algunos filtros sean
registrado en el *(router)*.

// TODO: graphic

Filtros pueden hacer una de tres opciones al manejar una petición:

1. Delegar el control hacia el siguiente *(handler)*, potencialmente después de modificar la petición.
2. Retornar una respuesta por sí mismo y no delegar el control a los siguientes *(handlers)*
3. Devolver un error

Estas opciones, y porqué cada una podría ser deseada, son discutidas abajo.

### *(Forward to Another Handler)* ¿Pasar al siguiente controlador?

El comportamiento más común para un filtro es pasar la petición al siguiente
*(handler)* después de haber modificado la petición o haber invocado algún
efecto colateral externo. Un caso común de uso es usar un filtro  como un
*(request logger)*:
...
Este filtro es *(straightfoward)*: invoca una acción externa (*(logging)*) y 
luego llama `forward()` para pasar el control al siguiente *(handler)* en el
*(pipeline)* (otro filtro y otro endpoint).

Debido a que los parámetros `req` y `res` en Plumber están basados en los
ambientes de R, estos exhiben un comportamiento "*(pass-by-reference)*". Esto
quiere decir que los cambios que sean hechos en un filtro en el objeto `req` o
`res` serán visibles para los otros filtros o endpoints que también están
relacionados con la misma petición o respuesta.

Un filtro similar puede cambiar algún estado en la respuesta o petición que se le ha dado
*¿?*
...
En este caso el objeto `req` será extendido para tener una propiedad adicional
llamada `username` la cual representa un valor revisado desde un *(cookie)*. 
Esta propiedad `req$username` estará disponible a todos los filtros subsecuentes
y endpoints procesando ésta petición. (Hacer valer que este ejemplo no es un
sistema seguro para autentificarse; revisar la [sección sobre uso de cookies para guardar un estado](#state-cookies) para una mayor discusión sobre el porqué). Una vez que
ha sido modificado el objeto del pediddo, pasará el control al siguiente 
*(handler)* usando `forward()`
