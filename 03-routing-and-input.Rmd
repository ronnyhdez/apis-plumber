...
```r
#* Escribe información sobre la solicitud entrante a la bitacora (log)
#* @filter logger
function(req){
...
Además de tener rutas de codificación fija, como `/hola`, los enlaces de Plumber pueden tener rutas dinamicas. Rutas dinamicas te permiten definir una serie de caminos más flexibles, contra los cuales tienen que coincidir tus enlaces.
...
Esta API usa el camino dinámico `/users/<id>` para buscar la concidencia entre cualquier solicitud que entra con la forma `/usuarios/` seguido por un elemento adicional del caminio como un numero o una letra. En este caso va a devolver información sobre el usuario si el usuario se encontró con un ID correspondiente, o un objeto vacio si no se encontro.

Puedes darle cualquier nombre a estos elementos del camino que te guste, pero toma en cuenta que el nombre usado en el camino dinámico tiene que coincidir con un parametro de la función (en este caso `id`). 

Puedes construir rutas dinámicas aún más complejas como:
...
En ambos ejemplos mostrados, el escrito en código y el dinámico, los parámetros
serán dados a la función como un *(string)* de caracteres.

## Rutas dinámicas escritas {#typed-dynamic-routes}

A menos que sea indicado de otra manera, todos los parámetros pasados a endoints de plumber desde consultas o caminos dinámicos serán cadenas de carácteres (strings). Por ejemplo, considere el siguiente API:

```{r, echo=FALSE, results='asis'}
include_r("files/apis/03-02-types.R")
```

Visitando http://localhost:8000/types/14 nos devolverá:
...
Si solamente tiene la intención de admitir un tipo particular de datos para un parámetro en particular en su ruta dinámica (dynamic route), puede especificar el tipo deseado en la misma ruta.
...
```
Especificar el tipo de algún elemento del camino dinámico también reducirá los caminos (paths) que coincidirán con el endpoint. Por ejemplo, el camino (path) `/users/123` coincidirá el primer endpoint, pero `/users/8e3k` no, ya que `8e3k` no es un entero.

Los siguientes detalles del mapeo del tipo de nombres que puede usar en sus tipos dinámicos y cómo estos mapean los tipos de datos en R.

Tipo en R  | Nombre en Plumber
------- | ---------
...
Plumber incluye un servidor de archivos estáticos que puede ser usado para albergar repositorios de activos estáticos (static assets) tal como JavaScript, CSS o archivos HTML. Estos servidores son bastante simples para configurar e integrar con su aplicación de plumber.
...
Este ejemplo expondrá el directorio local `./files/static` al camino por defecto `/public` en su servidor. Por lo que si usted tiene un archivo `./files/static/branding.html`, estará disponible en su servidor de Plumber en `/public/branding.html`.

Usted puede opcionalmente proveer un argumento adicional para configurar el camino público usado por su servidor. Por ejemplo:
...
Expondrá el directorio local `files/static` no en `/public`, pero en `/static`. De igual forma, para servir un principal `index.html`, puede también mapear `/files/static` a `/` usando

```r
#* @assets ./files/static /
list()
```
Esto permite servir su `/files/static/index.html` bajo el URL raíz http://localhost:8000/
